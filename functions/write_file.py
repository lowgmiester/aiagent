...
def write_file(working_directory, file_path, content):
    ...
    if not abs_path.startswith(abs_working):
        return {"error": f'Error: Cannot write to "{file_path}" as it is outside the permitted working directory'}
    ...
    with open(abs_path, "w") as f:
        f.write(content)
    return {"content": f'Successfully wrote to "{file_path}" ({len(content)} characters written)'}
